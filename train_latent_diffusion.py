...
def main(rank: int, world_size: int, save_path: str, args):
       ...
       encoder = DecisionNCE_encoder(args.encoder)
       cond_dim = encoder.v_dim * args.frames + encoder.l_dim
       input_dim = encoder.v_dim
       output_dim = encoder.v_dim
       vpmodel = IDQLDiffusion(input_dim, output_dim, cond_dim, device=rank).to(rank)
       vpagent = DDPM_BC_latent(vpmodel, schedule='vp', num_timesteps=50, mmencoder=encoder)      
